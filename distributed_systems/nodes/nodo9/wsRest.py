...
#ip del host
def ip():
    return {'ip': "172.17.0.10", 'port':'2008'}
...
def validar():
    ...
    if len(cpvecinos) == 0:
        ...
        if not any('ip' in d for d in send):
            send.append({'ip': '172.17.0.10'}) #CAMBIA ESTO POR LA IP LOCAL DEL HOST
            print("👾")
            try:
...
if __name__ == '__main__':
 app.run(host="172.17.0.10", port=2008)
