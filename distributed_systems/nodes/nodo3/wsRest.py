...
ipfather = None
ph = [
    {'1': 'Nodo 3: ¡Alegría, hermoso destello de los dioses,'}
    ]
vecinos_iter = None # datos de hijos a iterar 🦠 MODIFICAR

# Nodo 3: Frase1 = "¡Alegría, hermoso destello de los dioses,"
...
#ip del host
def ip():
    return {'ip': "172.17.0.4", 'port':'2002'}
...
def validar():
    ...
    if len(cpvecinos) == 0:
        ...
        if not any('ip' in d for d in send):
            send.append({'ip': '172.17.0.4'}) #CAMBIA ESTO POR LA IP LOCAL DEL HOST
            print("👾")
            try:
...
if __name__ == '__main__':
 app.run(host="172.17.0.4", port=2002)
