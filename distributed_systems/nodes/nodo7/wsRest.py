...
ipfather = None
ph = [
    {'8': 'Nodo 7: allí donde tu suave ala se posa.'}
    ]
vecinos_iter = None # datos de hijos a iterar 🦠 MODIFICAR

# Nodo 7: Frase8 = "allí donde tu suave ala se posa."
...
#ip del host
def ip():
    return {'ip': "172.17.0.8", 'port':'2006'}
vecinos_iter = None # datos de hijos a iterar 🦠 MODIFICAR
...
def validar():
    ...
    if len(cpvecinos) == 0:
        ...
        if not any('ip' in d for d in send):
            send.append({'ip': '172.17.0.8'}) #CAMBIA ESTO POR LA IP LOCAL DEL HOST
            print("👾")
            try:
...
if __name__ == '__main__':
 app.run(host="172.17.0.8", port=2006)
