...
ipfather = None
ph = [
    {'6': 'Nodo 8: lo que la acerba costumbre había separado;'}
    ]
vecinos_iter = None # datos de hijos a iterar 🦠 MODIFICAR

# Nodo 8: Frase6 = "lo que la acerba costumbre había separado;"
...
#ip del host
def ip():
    return {'ip': "172.17.0.9", 'port':'2007'}
...
def validar():
    ...
    if len(cpvecinos) == 0:
        ...
        if not any('ip' in d for d in send):
            send.append({'ip': '172.17.0.9'}) #CAMBIA ESTO POR LA IP LOCAL DEL HOST
            print("👾")
            try:
...
if __name__ == '__main__':
    app.run(host="172.17.0.9", port=2007)
