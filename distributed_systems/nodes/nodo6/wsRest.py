...
ipfather = None
ph = [
    {'3': 'Nodo 6: Ebrios de entusiasmo entramos,'}
    ]
vecinos_iter = None # datos de hijos a iterar 🦠 MODIFICAR

# Nodo 6: Frase3 = "Ebrios de entusiasmo entramos,"
...
#ip del host
def ip():
    return {'ip': "172.17.0.7", 'port':'2005'}
...
def validar():
    ...
    if len(cpvecinos) == 0:
        ...
        if not any('ip' in d for d in send):
            send.append({'ip': '172.17.0.7'}) #CAMBIA ESTO POR LA IP LOCAL DEL HOST
            print("👾")
            try:
...
if __name__ == '__main__':
 app.run(host="172.17.0.7", port=2005)
